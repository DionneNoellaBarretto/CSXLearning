...
// Challenge Exercise Link: http://csbin.io/jsep/2
...
function buildSentence(word1, word2, word3) {
    var newWord = word1[0].toUpperCase();
    for(var i = 1; i < word1.length; i++) 
        {
            newWord += word1[i];
        }
    let result = newWord + ' ' + word2 + ' ' + word3 + '.';
    console.log(result);
}
...
Hint: Remember that each character/letter in a string has an index position that you can access with brackets: []
  * =========== */
  ...
  function lastLetter(word) {
    //   i initially solved it like this but then realized since its only 1 letter (last letter) in the word we dont really need a for loop! 
    // for (let i = word.length - 1; i < word.length; i++) {
    //   var word = word[i]
    // }
    var word = word[word.length - 1];
